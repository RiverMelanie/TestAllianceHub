<template>
	<el-form :inline="true" :model="queryParams" class="search-form">
		...
		<el-form-item>
			...
			<!-- 添加待审核筛选按钮 -->
			<el-button type="warning" @click="filterPendingCourses">待审核课程</el-button>
	...
	<el-table :data="tableData" style="width: 100%">
		...
		<el-table-column label="操作" min-width="150">
			<template #default="scope">
				...
				<el-button v-if="scope.row.status === 0" link type="success" size="small"
					@click="approveCourse(scope.row.course_id)">
					通过
				</el-button>
				<el-button v-if="scope.row.status === 0" link type="danger" size="small"
...
<script setup lang="ts">
	...
	const filterPendingCourses = () => {
		queryParams.value.status = 0 // 设置状态为待审核
		handleQuery() // 执行查询
	}
	...
	const search = () => {
		...
		axios.get("http://localhost:8080/getallcrs", {
			params: {
				...
				pageSize: 10,
				courseName: queryParams.value.courseName.trim(),
				sortOrder: queryParams.value.sort_order ? parseInt(queryParams.value.sort_order) : null,
				status: queryParams.value.status // 添加状态参数
			}
		})
			.then(res => {
...
const addcrs = () => {
  ...
  axios.post("http://localhost:8080/addcrs", form.value)
	...
	const approveCourse = (courseId : number) => {
		ElMessageBox.confirm('确认通过该课程审核吗?', '提示', {
			confirmButtonText: '确定',
			cancelButtonText: '取消',
			type: 'warning'
		}).then(() => {
			updateCourseStatus(courseId, 1, '审核通过成功');
		}).catch(() => {
	...
	const rejectCourse = (courseId : number) => {
		ElMessageBox.confirm('确认不通过该课程审核吗?', '提示', {
			confirmButtonText: '确定',
			cancelButtonText: '取消',
			type: 'warning'
		}).then(() => {
			updateCourseStatus(courseId, 2, '审核不通过成功');
		}).catch(() => {
			ElMessage.info('已取消操作');
		});
	};

	const updateCourseStatus = (courseId : number, status : number, successMessage : string) => {
		axios.post("http://localhost:8080/updateCourseStatus", {
			course_id: courseId,
			status: status
		})
			.then(res => {
